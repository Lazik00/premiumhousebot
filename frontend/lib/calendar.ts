...
export function formatMonthLabel(monthDate: Date, locale: string): string {
...
export function isNightBlocked(dateKey: string, ranges: BlockedRange[]): boolean {
    return ranges.some((range) => range.start_date <= dateKey && dateKey < range.end_date);
}

export function rangeHasBlockedNights(startDate: string, endDate: string, ranges: BlockedRange[]): boolean {
...
export function isCheckoutCandidate(dateKey: string, startDate: string | null, ranges: BlockedRange[]): boolean {
